 ...
 *
 * <p>Note: if the native libraries cannot be loaded, an ExceptionInInitializerError will be thrown
 * at first use of this class.
 *
 * @author Taro L. Saito
 */
public class Snappy {
  /** An instance of SnappyNative */
  private static final SnappyNative impl;
  ...
  /**
   * Clean up a temporary file (native lib) generated by snappy-java. General users do not need to
   * call this method, since the native library extracted in snappy-java is deleted upon JVM
   * termination (vie deleteOnExit()). This method is useful when using a J2EE container, which will
   * restart servlet containers multiple times without restarting JVM.
   */
  ...
  /**
   * High-level API for compressing the input byte array. This method performs array copy to
   * generate the result. If you want to reduce the memory copy cost, use {@link #compress(byte[],
   * int, int, byte[], int)} or {@link #compress(ByteBuffer, ByteBuffer)}.
   *
  ...
  /**
   * Compress the input buffer content in [inputOffset, ...inputOffset+inputLength) then output to
  ...
  public static int compress(ByteBuffer uncompressed, ByteBuffer compressed) throws IOException {
    ...
    if (!compressed.isDirect()) {
      throw new SnappyError(
          SnappyErrorCode.NOT_A_DIRECT_BUFFER, "destination is not a direct buffer");
    }
    ...
    int compressedSize =
        impl.rawCompress(uncompressed, uPos, uLen, compressed, compressed.position());
   ...
   * successfully. Does not return the uncompressed data. Takes time proportional to the input
   * length, but is usually at least a factor of four faster than actual decompression.
   */
  ...
  /**
   * Compress the input buffer [offset,... ,offset+length) contents, then write the compressed data
   * to the output buffer[offset, ...)
   *
   ...
   * @throws IOException
   */
  public static int rawCompress(
      Object input, int inputOffset, int inputLength, byte[] output, int outputOffset)
      throws IOException {
  ...
  /**
   * Uncompress the content in the input buffer. The uncompressed data is written to the output
   * buffer.
   *
   * <p>Note that if you pass the wrong data or the range [inputOffset, inputOffset + inputLength)
   * that cannot be uncompressed, your JVM might crash due to the access violation exception issued
   * in the native code written in C++. To avoid this type of crash, use {@link
   ...
   */
  public static int rawUncompress(
      byte[] input, int inputOffset, int inputLength, Object output, int outputOffset)
      throws IOException {
    if (input == null || output == null) {
      throw new NullPointerException("input or output is null");
    }
  ...
  /**
   * Uncompress the content in the input buffer. The result is dumped to the specified output
   * buffer.
   *
   * <p>Note that if you pass the wrong data or the range [pos(), limit()) that cannot be
   * uncompressed, your JVM might crash due to the access violation exception issued in the native
   * code written in C++. To avoid this type of crash, use {@link
  ...
  public static int uncompress(ByteBuffer compressed, ByteBuffer uncompressed) throws IOException {
    ...
    int decompressedSize =
        impl.rawUncompress(compressed, cPos, cLen, uncompressed, uncompressed.position());
   ...
   * @throws IOException when failed to uncompress the given input. The error code is {@link
