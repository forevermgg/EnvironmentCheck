...
public abstract class QtNativeViewModel extends ViewModel implements NativeObject {
  ...
  private final ReentrantReadWriteLock shellHolderLock = new ReentrantReadWriteLock();
  @NonNull private final Looper mainLooper;
  public ConcurrentHashMap<Integer, ViewModelHandleProp> setPropDispatcherByInterface =
      new ConcurrentHashMap();
  public ConcurrentHashMap<Integer, Consumer<String>> setPropDispatcherByFunction =
      new ConcurrentHashMap<>();
  ...
  public void showToast(byte[] params) {
    ...
    Timber.e(
        ...
        toastParams.getContent(), toastParams.getDuration());
    ArchTaskExecutor.getInstance()
        .postToMainThread(
            () -> {
              if (toastParams.getDuration() > 0) {
                ...
                Toast.makeText(
                        ContextProvider.get().getContext(),
                        toastParams.getContent(),
                        toastParams.getSystemsDuration())
                    .show();
              }
            });
  }
  ...
  public void setProp(int key, String value) {
    ...
    synchronized (mutex) {
      if (value != null) {
        Objects.requireNonNull(setPropDispatcherByFunction.get(key)).accept(value);
        Objects.requireNonNull(setPropDispatcherByInterface.get(key))
