...
FOREVER_THREAD_LOCAL ThreadLocalUniquePtr<MessageLoop> tls_message_loop;
...
FOREVER::RefPtr<FOREVER::TaskRunner> MessageLoop::GetTaskRunner() const {
  return task_runner_;
}

FOREVER::RefPtr<MessageLoopImpl> MessageLoop::GetLoopImpl() const {
  return loop_;
}

void MessageLoop::AddTaskObserver(intptr_t key, const FOREVER::closure& callback) {
