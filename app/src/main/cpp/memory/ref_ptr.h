...
#ifndef FOREVER_MEMORY_REF_PTR_H_
#define FOREVER_MEMORY_REF_PTR_H_
...
// RefPtr
// 是引用指针的实现类，需要关注的是各类构造、赋值、析构函数的行为。构造函数、拷贝构造函数需要
// AddRef，转移构造函数不需要 AddRef；析构函数需要 Release；拷贝赋值函数AddRef
// 新的对象，Release 久的对象；转移赋值函数不需要变更引用计数。
template <typename T>
class RefPtr final {
 public:
  RefPtr() : ptr_(nullptr) {}
...
namespace std {
template <typename T>
struct hash<FOREVER::RefPtr<T>> {
  using argument_type = FOREVER::RefPtr<T>;
...
#endif  // FOREVER_MEMORY_REF_PTR_H_
