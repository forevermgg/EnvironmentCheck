...
void notify_fd(int write_fd) {
  ...
  // If the pipe's buffer is full, ALOOPER_EVENT_INPUT will be triggered anyway.
  // Also the buffer clearing happens before calling the callback. So after this
  // call, the callback will be called. Just return here.
  if (ret != 0) {
    int err = errno;
    if (err != EAGAIN) {
...
class ALooperScheduler : public util::Scheduler,
                         public std::enable_shared_from_this<ALooperScheduler> {
 public:
  ALooperScheduler() {
    if (m_looper) ALooper_acquire(m_looper);
  }
  ...
  ~ALooperScheduler() {
    ...
    if (m_initialized) {
      ...
      ::close(m_message_pipe.read);
      {
        ...
        s_live_schedulers.erase(std::remove(s_live_schedulers.begin(),
                                            s_live_schedulers.end(), this),
 ...
 private:
  ...
  void init() {
    ...
    int message_pipe[2];
    // pipe2 became part of bionic from API 9. But there are some devices with
    // API > 10 that still have problems. See
    // https://github.com/realm/realm-java/issues/3945 .
    if (pipe(message_pipe)) {
      int err = errno;
      LOGE("could not create WeakRealmNotifier ALooper message pipe: %s.",
           strerror(err));
      return;
    }
    if (fcntl(message_pipe[0], F_SETFL, O_NONBLOCK) == -1 ||
        fcntl(message_pipe[1], F_SETFL, O_NONBLOCK) == -1) {
      int err = errno;
      LOGE("could not set ALooper message pipe non-blocking: %s.",
           strerror(err));
      // It still works in blocking mode.
    }

    if (ALooper_addFd(m_looper, message_pipe[0], ALOOPER_POLL_CALLBACK,
                      ALOOPER_EVENT_INPUT, &looper_callback, this) != 1) {
  ...
  static int looper_callback(int fd, int events, void* data) {
    if ((events & ALOOPER_EVENT_INPUT) != 0) {
      ...
      {
        std::lock_guard lock(s_schedulers_mutex);
        if (std::find(s_live_schedulers.begin(), s_live_schedulers.end(),
                      data) != s_live_schedulers.end()) {
          // Even if the weak_ptr can be found in the list, the object still can
          // be destroyed in between. But share_ptr can ensure we either have a
          // valid pointer or the object has gone.
          shared = static_cast<ALooperScheduler*>(data)->shared_from_this();
        }
      }
      if (shared) {
        // Clear the buffer. Note that there might be a small chance than more
        // than 1024 bytes left in the pipe, but it is OK. Since we also want to
        // support blocking read here. Clear here instead of in the notify is
        // because of whenever there are bytes left in the pipe, the
        // ALOOPER_EVENT_INPUT will be triggered.
        std::vector<uint8_t> buff(1024);
        read(fd, buff.data(), buff.size());
        // By holding a shared_ptr, this object won't be destroyed in the
        // m_callback.
