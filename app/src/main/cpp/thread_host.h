...
#ifndef FOREVER_SHELL_COMMON_THREAD_HOST_H_
#define FOREVER_SHELL_COMMON_THREAD_HOST_H_
...
using ThreadConfig = FOREVER::Thread::ThreadConfig;
using ThreadConfigSetter = FOREVER::Thread::ThreadConfigSetter;
...
struct ThreadHost {
  ...
  struct ThreadHostConfig {
    explicit ThreadHostConfig(const ThreadConfigSetter& setter =
                                  FOREVER::Thread::SetCurrentThreadName)
        : type_mask(0), config_setter(setter) {}

    ThreadHostConfig(const std::string& name_prefix, uint64_t mask,
                     const ThreadConfigSetter& setter =
                         FOREVER::Thread::SetCurrentThreadName)
        : type_mask(mask), name_prefix(name_prefix), config_setter(setter) {}

    explicit ThreadHostConfig(uint64_t mask,
                              const ThreadConfigSetter& setter =
                                  FOREVER::Thread::SetCurrentThreadName)
  ...
  std::string name_prefix;
  std::unique_ptr<FOREVER::Thread> platform_thread;
  std::unique_ptr<FOREVER::Thread> ui_thread;
  std::unique_ptr<FOREVER::Thread> raster_thread;
  std::unique_ptr<FOREVER::Thread> io_thread;
  std::unique_ptr<FOREVER::Thread> profiler_thread;
 ...
 private:
  std::unique_ptr<FOREVER::Thread> CreateThread(
...
#endif  // FOREVER_SHELL_COMMON_THREAD_HOST_H_
