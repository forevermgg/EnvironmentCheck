...
#ifndef FOREVER_SRC_INCLUDE_FOREVER_INTERNAL_TYPE_TRAITS_H_
#define FOREVER_SRC_INCLUDE_FOREVER_INTERNAL_TYPE_TRAITS_H_
...
#ifdef STLPORT
#define FOREVER_TYPE_TRAITS_NS std::tr1
#else
#define FOREVER_TYPE_TRAITS_NS std
#endif

template <typename T>
using decay = FOREVER_TYPE_TRAITS_NS::decay<T>;

template <typename T>
using decay_t = typename decay<T>::type;

template <bool value, typename T = void>
using enable_if = FOREVER_TYPE_TRAITS_NS::enable_if<value, T>;

template <typename T>
using is_floating_point = FOREVER_TYPE_TRAITS_NS::is_floating_point<T>;

template <typename T>
using is_integral = FOREVER_TYPE_TRAITS_NS::is_integral<T>;

template <typename T, typename U>
using is_same = FOREVER_TYPE_TRAITS_NS::is_same<T, U>;

template <typename T, T value>
using integral_constant = FOREVER_TYPE_TRAITS_NS::integral_constant<T, value>;

using true_type = FOREVER_TYPE_TRAITS_NS::true_type;
using false_type = FOREVER_TYPE_TRAITS_NS::false_type;

#undef FOREVER_TYPE_TRAITS_NS
...
#endif  // FOREVER_SRC_INCLUDE_FOREVER_INTERNAL_TYPE_TRAITS_H_
