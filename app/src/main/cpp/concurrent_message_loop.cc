...
ConcurrentMessageLoop::ConcurrentMessageLoop(size_t worker_count)
  ...
  for (size_t i = 0; i < worker_count_; ++i) {
    workers_.emplace_back([i, this]() {
      FOREVER::Thread::SetCurrentThreadName(FOREVER::Thread::ThreadConfig(
...
void ConcurrentMessageLoop::PostTask(const FOREVER::closure& task) {
  if (!task) {
    return;
  }
...
void ConcurrentMessageLoop::WorkerMain() {
  while (true) {
    ...
    FOREVER::closure task;
    std::vector<FOREVER::closure> thread_tasks;
    ...
    if (HasThreadTasksLocked()) {
      ...
      FOREVER_DCHECK(!HasThreadTasksLocked());
    }
...
void ConcurrentMessageLoop::PostTaskToAllWorkers(FOREVER::closure task) {
  if (!task) {
    return;
  }
...
std::vector<FOREVER::closure> ConcurrentMessageLoop::GetThreadTasksLocked() {
  auto found = thread_tasks_.find(std::this_thread::get_id());
  FOREVER_DCHECK(found != thread_tasks_.end());
  std::vector<FOREVER::closure> pending_tasks;
...
void ConcurrentTaskRunner::PostTask(const FOREVER::closure& task) {
  if (!task) {
    return;
  }
