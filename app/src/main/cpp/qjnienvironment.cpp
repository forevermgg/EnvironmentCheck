...
/*!
    ...
    \inmodule QtCore
    \since 6.1
    \brief The QJniEnvironment class provides access to the JNI Environment
   (JNIEnv).

    When using JNI, the \l {JNI tips: JavaVM and JNIEnv}{JNIEnv} class is a
   pointer to a function table and a member function for each JNI function that
   indirects through the table. \c JNIEnv provides most of the JNI functions.
   Every C++ native function receives a \c JNIEnv as the first argument. The JNI
   environment cannot be shared between threads.

    Since \c JNIEnv doesn't do much error checking, such as exception checking
   and clearing, QJniEnvironment allows you to do that easily.
...
/*!
    ...
    Constructs a new JNI Environment object and attaches the current thread to
   the Java VM.
*/
QJniEnvironment::QJniEnvironment() : d(new QJniEnvironmentPrivate{}) {
...
/*!
    ...
    Detaches the current thread from the Java VM and destroys the
   QJniEnvironment object. This will clear any pending exception by calling
   checkAndClearExceptions().
*/
QJniEnvironment::~QJniEnvironment() { checkAndClearExceptions(); }
...
/*!
    ...
    \since 6.2
*/
bool QJniEnvironment::isValid() const { return d->jniEnv; }
...
*/
JNIEnv *QJniEnvironment::operator->() const { return d->jniEnv; }
...
*/
JNIEnv *QJniEnvironment::jniEnv() const { return d->jniEnv; }
...
/*!
    ...
    call. Additionally, this call looks for internally cached classes first
   before doing a JNI call, and returns such a class if found. The following
   code snippet creates an instance of the class \c CustomClass and then calls
   the \c printFromJava() method:

    \code
    QJniEnvironment env;
    jclass javaClass =
   env.findClass("org/qtproject/example/android/CustomClass"); QJniObject
   javaMessage = QJniObject::fromString("findClass example");
    QJniObject::callStaticMethod<void>(javaClass, "printFromJava",
                                       "(Ljava/lang/String;)V",
   javaMessage.object<jstring>()); \endcode
...
jmethodID QJniEnvironment::findMethod(jclass clazz, const char *methodName,
  ...
  if (clazz) {
    ...
    if (!checkAndClearExceptions(d->jniEnv)) return id;
  }
...
/*!
    ...
    QJniEnvironment env;
    jclass javaClass =
   env.findClass("org/qtproject/example/android/CustomClass"); jmethodID
   methodId = env.findStaticMethod(javaClass, "staticJavaMethod",
                                              "(Ljava/lang/String;)V");
    ...
    \since 6.2
*/
jmethodID QJniEnvironment::findStaticMethod(jclass clazz,
...
/*!
    ...
    \since 6.2
*/
jfieldID QJniEnvironment::findField(jclass clazz, const char *fieldName,
                                    const char *signature) {
  if (clazz) {
    jfieldID id = d->jniEnv->GetFieldID(clazz, fieldName, signature);
    if (!checkAndClearExceptions()) return id;
  }
...
/*!
    ...
    \since 6.2
*/
jfieldID QJniEnvironment::findStaticField(jclass clazz, const char *fieldName,
                                          const char *signature) {
  if (clazz) {
...
*/
JavaVM *QJniEnvironment::javaVM() { return QtAndroidPrivate::javaVM(); }
...
/*!
    ...
    const JNINativeMethod methods[] =
                            {{"callNativeOne", "(I)V", reinterpret_cast<void
   *>(fromJavaOne)},
                            {"callNativeTwo", "(I)V", reinterpret_cast<void
   *>(fromJavaTwo)}}; QJniEnvironment env;
    env.registerNativeMethods("org/qtproject/android/TestJavaClass", methods,
   2); \endcode
*/
bool QJniEnvironment::registerNativeMethods(const char *className,
                                            const JNINativeMethod methods[],
                                            int size) {
...
/*!
    ...
    \code
    JNINativeMethod methods[] {{"callNativeOne", "(I)V", reinterpret_cast<void
   *>(fromJavaOne)},
                               {"callNativeTwo", "(I)V", reinterpret_cast<void
   *>(fromJavaTwo)}}; QJniEnvironment env; jclass clazz =
   env.findClass("org/qtproject/android/TestJavaClass");
    env.registerNativeMethods(clazz, methods, 2);
    \endcode
*/
bool QJniEnvironment::registerNativeMethods(jclass clazz,
                                            const JNINativeMethod methods[],
                                            int size) {
...
/*!
    \fn QJniEnvironment::checkAndClearExceptions(OutputMode outputMode =
   OutputMode::Verbose)
    ...
    \c JNIEnv calls that can throw an exception, see \l {Java: JNI
   Functions}{JNI Functions}.

    \return \c true when a pending exception was cleared.
*/
bool QJniEnvironment::checkAndClearExceptions(
    QJniEnvironment::OutputMode outputMode) {
  if ((d->jniEnv->ExceptionCheck())) {
    if (outputMode != OutputMode::Silent) d->jniEnv->ExceptionDescribe();
...
/*!
    \fn QJniEnvironment::checkAndClearExceptions(JNIEnv *env, OutputMode
   outputMode = OutputMode::Verbose)
    ...
    already have a \c JNIEnv pointer such as in a native function
   implementation.
...
*/
bool QJniEnvironment::checkAndClearExceptions(
    JNIEnv *env, QJniEnvironment::OutputMode outputMode) {
  if ((env->ExceptionCheck())) {
    if (outputMode != OutputMode::Silent) env->ExceptionDescribe();
    env->ExceptionClear();
