...
class QJniObject {
 public:
  ...
  T callMethod(const char *methodName, const char *signature, ...) const {
    ...
    if (id) {
      ...
      va_end(args);
      if (env.checkAndClearExceptions()) res = {};
    }
    return res;
  }

  template <>
  void callMethod<void>(const char *methodName, const char *signature,
                        ...) const {
    QJniEnvironment env;
    jmethodID id = getCachedMethodID(env.jniEnv(), methodName, signature);
    if (id) {
  ...
  T callMethod(const char *methodName) const {
    ...
    std::stringstream methodNameCombined;
    methodNameCombined << "()" << signature;
    /*return callMethod<T>(methodName,
                         QByteArray(signature).prepend("()").constData());*/
    return callMethod<T>(methodName, methodNameCombined.str());
  }
  ...
  QJniObject callObjectMethod(const char *methodName) const {
    ...
    std::stringstream methodNameCombined;
    methodNameCombined << "()" << signature;
    return callObjectMethod(methodName, methodNameCombined.str().c_str());
    /*return callObjectMethod(methodName,
                            QByteArray(signature).prepend("()").constData());*/
  }

  QJniObject callObjectMethod(const char *methodName, const char *signature,
                              ...) const;
  ...
  static T callStaticMethod(const char *className, const char *methodName) {
    ...
    std::stringstream methodNameCombined;
    methodNameCombined << "()" << signature;
    return callStaticMethod<T>(className, methodName,
                               methodNameCombined.str().c_str());
    /*return callStaticMethod<T>(className, methodName,
                               QByteArray(signature).prepend("()").constData());*/
  }
  ...
  static T callStaticMethod(jclass clazz, const char *methodName) {
    ...
    std::stringstream methodNameCombined;
    methodNameCombined << "()" << signature;
    return callStaticMethod<T>(clazz, methodName,
                               methodNameCombined.str().c_str());
    /*return callStaticMethod<T>(clazz, methodName,
                               QByteArray(signature).prepend("()").constData());*/
  }
  ...
  static QJniObject callStaticObjectMethod(jclass clazz,
    ...
    std::stringstream methodNameCombined;
    methodNameCombined << "()" << signature;
    return callStaticObjectMethod(clazz, methodName,
                                  methodNameCombined.str().c_str());
    /*return callStaticObjectMethod(
            clazz, methodName,
       QByteArray(signature).prepend("()").constData());*/
  ...
  template <typename T>
  static T getStaticField(const std::string &className, const char *fieldName) {
    assertJniPrimitiveType<T>();
    QJniEnvironment env;
    jclass clazz = QJniObject::loadClass(className, env.jniEnv());
    if (!clazz) return 0;
    ...
    constexpr const char *signature = getTypeSignature<T>();
    std::string name = QJniObject::toBinaryEncClassName(className);
    jfieldID id =
        getCachedFieldID(env.jniEnv(), clazz, name, fieldName, signature, true);
    if (!id) return 0;
  ...
  template <typename T>
  static void setStaticField(const std::string &className,
  ...
  static QJniObject fromString(const std::string &string);
 ...
 private:
  ...
  static jfieldID getCachedFieldID(JNIEnv *env, jclass clazz,
                                   ...
                                   bool isStatic = false);
  jfieldID getCachedFieldID(JNIEnv *env, const char *name,
                            const char *signature, bool isStatic = false) const;
  static jmethodID getCachedMethodID(JNIEnv *env, jclass clazz,
                                     const std::string &className,
                                     const char *name, const char *signature,
                                     bool isStatic = false);
  jmethodID getCachedMethodID(JNIEnv *env, const char *name,
                              const char *signature,
  ...
  static constexpr void assertJniPrimitiveType() {
    if constexpr (!isJniPrimitiveType<T>()) staticAssertTypeMismatch();
  }
  ...
  template <typename T>
  static constexpr void callStaticMethodForType(JNIEnv *env, T &res,
  ...
  template <typename T>
  static constexpr void setFieldForType(JNIEnv *env, jobject obj, jfieldID id,
                                        T value) {
    if constexpr (std::is_same<T, jboolean>::value)
